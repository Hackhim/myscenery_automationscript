...
class PrinterRecord(Base):
    ...
    filament = SingleRecordLinkField('Filament', linked_class='farm.model.filament.FilamentRecord')
    profile = SingleRecordLinkField('PrinterProfile', linked_class='farm.model.printer.PrinterProfileRecord')
    group = SingleRecordLinkField('PrinterGroup', linked_class='farm.model.printer.PrinterGroupRecord')

    @classmethod
    def get_ready(cls):
        ready_printers = cls.objects.filter(automated=True, status=Status.OPERATIONAL, clean_plate=True, filament__empty=False, profile__empty=False, group__empty=True)
        return ready_printers

    @classmethod
    def get_ready_in_group(cls):
        ready_printers = cls.objects.filter(automated=True, status=Status.OPERATIONAL, clean_plate=True, profile__empty=False, group__empty=False)
        return ready_printers
...
class Printer:
    ...
    def create_octoprint_connection(self):
        try:
            self.octoprint = Octoprint(url=self.record.url, api_key=self.record.octoprint_api_key, timeout=self.octoprint_timeout)
        except Exception as e:
            self.octoprint = None
        ...
        if self.is_disconnected():
            ...
            if not (raw_octo_status in status_from_octoprint.keys()):
                if 'Failed to autodetect serial port' in raw_octo_status:
                    ...
                    raise Exception(f'Octoprint status not found: "{raw_octo_status}", {self.__repr__()}')
            else:
            ...
            if self.is_printing() and octo_status == Status.OPERATIONAL:
                ...
                pass #Do not update status
            elif (self.is_disconnected() or self.is_operational()) and octo_status == Status.OPERATIONAL and not self.record.clean_plate:
                self.set_status(Status.HARVEST)
            elif self.is_printing() and octo_status == Status.PRINTING and self.record.clean_plate:
                self.record.clean_plate = False
                self.record.save()
            else:
    ...
    def is_disconnected(self):
        return self.record.status == Status.DISCONNECTED or self.record.status == Status.SERIAL_PORT_ERROR
    ...
    def fit_in_bed(self, printfile):
        return (printfile.size_x <= self.record.profile.size_x
    ...
    def print(self, remote_filename):
        try:
            self.octoprint.select(location=f'{Printer.UPLOAD_DIR}/{remote_filename}', print=True)
            self.refresh_status()
            return True
        except Exception as e:
            raise(Exception(f'Error while launching {remote_filename} on {self}:\n{e}'))
            return False
